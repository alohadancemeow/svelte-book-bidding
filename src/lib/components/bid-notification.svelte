<script lang="ts">
  ...
  $effect(() => {
    const unsubscribe = realtime.updates.subscribe(updates => {
      if (updates.length > 0) {
        ...
        addNotification(
          `${latest.bidder} bid $${latest.amount.toLocaleString()} on auction #${latest.auctionId}`,
          'info'
        );
      }
    });
  ...
  function addNotification(message: string, type: 'info' | 'success' | 'warning' = 'info') {
    const id = Math.random().toString(36);
    notifications.push({ id, message, type, timestamp: new Date().toLocaleTimeString() });
